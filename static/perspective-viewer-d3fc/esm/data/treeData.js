...
export function treeData(settings) {
  ...
  const data = Object.entries(sets).map(set => {
    ...
    chartData.each(d => {
      d.current = d;
      d.mainValues = settings.realValues.length === 1 || settings.realValues[1] === null && settings.realValues[2] === null ? d.value : [d.value, d.data.color].concat(d.data.tooltip || []).filter(x => x !== null);
