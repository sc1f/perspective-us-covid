...
export class ActionElement extends DomElement {
  ...
  _save_computed_expression(event) {
    const expression = event.detail.expression; // `computed-columns` stores the raw expression typed by the user.
  ...
  async _type_check_computed_expression(event) {
    ...
    const schema = await this._table.computed_schema(parsed); // Look at the failing values, and get their expected types
  ...
  _column_visibility_clicked(ev) {
    ...
    if (is_active) {
      ...
      if (!cols[i] || !((_cols$i = cols[i]) === null || _cols$i === void 0 ? void 0 : _cols$i.classList.contains("null-column"))) {
