...
export class DomElement extends PerspectiveElement {
  ...
  _update_computed_column_view(computed_schema) {
    ...
    for (const cc of computed_columns) {
      const name = cc.column; // Check for whether the computed column is in the attribute but
  ...
  _reset_computed_column_view(computed_columns) {
    ...
    const computed_names = computed_columns.map(x => x.column); // Remove computed columns from all
    ...
    const filters = this._get_view_filters().filter(x => !computed_names.includes(x[0])); // Aggregates as an array is from the attribute API
    ...
    this.setAttribute("filters", JSON.stringify(filters)); // Remove inactive computed columns
  ...
  _update_column_list(columns, container, callback, accessor) {
    ...
    const active_columns = Array.prototype.slice.call(container.children); // Make sure that the `computed` attribute is set on computed columns
