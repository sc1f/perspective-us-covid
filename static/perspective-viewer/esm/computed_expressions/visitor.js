...
export class ComputedExpressionColumnVisitor extends base_visitor {
  ...
  OperatorComputedColumn(ctx, computed_columns) {
    ...
    let column_name = COMPUTED_FUNCTION_FORMATTERS[operator](left, right); // Use custom name if provided through `AS/as/As`
  ...
  FunctionComputedColumn(ctx, computed_columns) {
    const fn = this.visit(ctx.Function); // Functions have 1...n parameters
    ...
    let column_name = COMPUTED_FUNCTION_FORMATTERS[fn](...input_columns); // Use custom name if provided through `AS/as/As`
